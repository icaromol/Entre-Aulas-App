    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 pb-8 px-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-[#153b50]">Quanto tempo você tem hoje?</h2>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`text-lg font-bold transition ${unchanged ? "text-gray-400" : "text-[#153b50]"}`}>
            {fmtMin(selected)}
          </span>
        </div>
        ...
        <input
          ...
          className="w-full accent-[#153b50] mb-6"
        />

        <button
          disabled={unchanged}
          onClick={() => onConfirm(selected)}
          className="w-full py-3 rounded-xl bg-[#153b50] text-white text-sm font-semibold hover:bg-[#153b50]/90 transition disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Adaptar meu plano
        </button>
      </div>
