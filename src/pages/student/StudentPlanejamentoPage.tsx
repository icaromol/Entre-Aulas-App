...
export default function StudentPlanejamentoPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const completedPieces = (allPieces ?? []).filter(p => p.status === 'completed').map(p => ({ pieceId: p.id, pieceTitle: p.title, difficulty: p.difficulty }))

      const plan = generatePlan({ studentLevel, weekStart, horizon, availability, programs: resolvedPrograms, maintenance: { enabled: maintenanceEnabled, budgetPercent: maintenanceBudget, completedPieces } })
  ...
  function addTaskToDay(date: string, dow: number, ws: string, task: PlannedTask) {
    setEditableDays(prev => {
      ...
      return [...prev, { weekStart: ws, dayOfWeek: dow, date, minutesAvailable: 0, slots: 0, minutesUsed: task.durationMinutes, tasks: [task] }]
    })
    setAddingToDay(null)
    setAddCustomTitle('')
        ...
        {programs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-400 max-w-xs leading-relaxed mb-5">
              Crie ao menos um programa no seu repertório antes de gerar o planejamento.
            </p>
  ...
  return (
    <StudentLayout>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[{ label: 'Tarefas', value: totalScheduled }, { label: 'Semanas', value: weekStarts.length }, { label: 'Minutos', value: totalMinutes }].map(s => (
      ...
      <div ref={scrollRef}
        ...
        onMouseUp={onBoardMouseUp} onMouseLeave={onBoardMouseLeave}>
        <div className="flex flex-col md:flex-row md:min-w-max gap-3">
          {columns.map(col => {
            ...
            return (
              <div key={col.date} className={`w-full md:flex-none md:w-80 rounded-2xl border overflow-hidden flex flex-col ${isAvailable ? 'bg-white border-gray-100' : 'bg-gray-50 border-gray-100'}`}>
                <div className="px-4 py-3 border-b flex items-center justify-between bg-gray-50 border-gray-100">
                  ...
                  {minutesUsed > 0 && <span className="text-xs font-semibold text-[#4A90C4]">{minutesUsed} min</span>}
                </div>
                ...
                <div className="px-3 pb-3">
                  <button onClick={() => { setAddingToDay({ date: col.date, dow: col.dow, weekStart: currentWS }); setAddTab('program') }}
      ...
      <div className="flex gap-3 pb-8">
        <Button onClick={() => setStep('config')} variant="outline" className="flex-1 rounded-xl border-gray-200 text-gray-600">Voltar</Button>
          ...
          <div className="bg-white rounded-t-2xl px-6 pt-6 pb-8 w-full max-w-lg mx-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => deleteTask(editingTask.date, editingTask.idx)}
                className="flex-1 py-3 rounded-xl border border-red-100 text-red-400 text-sm font-semibold hover:bg-red-50 transition">Excluir tarefa</button>
              <button onClick={() => setEditingTask(null)}
                className="flex-1 py-3 rounded-xl bg-[#1E3A5F] text-white text-sm font-semibold">Fechar</button>
            </div>
          </div>
        </div>
      )}
            ...
            {addTab === 'program' ? (
                  ...
                  : pickerItems.map((item, idx) => (
                    <button key={idx}
                      onClick={() => addTaskToDay(addingToDay.date, addingToDay.dow, addingToDay.weekStart, {
                        pieceId: item.pieceId, exerciseId: item.exerciseId,
                        sourceType: item.sourceType, sourceTitle: item.title,
                        programId: item.programId, programTitle: item.programTitle,
                        durationMinutes: pomodoroWork, isMaintenance: false, score: 0,
                      })}
                      className="w-full text-left p-3 rounded-xl border border-gray-100 hover:border-[#4A90C4] transition">
                      <p className="text-sm font-medium text-gray-800 truncate">{item.title}</p>
                      <p className="text-xs text-gray-400 truncate mt-0.5">{item.programTitle}</p>
                    </button>
                  ))
                }
              </div>
              ...
              <div className="px-6 pb-8 shrink-0 space-y-5">
                <div className="space-y-1.5">
                  ...
                  <input value={addCustomTitle} onChange={e => setAddCustomTitle(e.target.value)}
                ...
                <Button disabled={!addCustomTitle.trim()}
                  onClick={() => addTaskToDay(addingToDay.date, addingToDay.dow, addingToDay.weekStart, {
                    pieceId: null, exerciseId: null, sourceType: 'piece',
                    sourceTitle: addCustomTitle.trim(), programId: null, programTitle: 'Personalizado',
                    durationMinutes: addCustomDuration, isMaintenance: false, score: 0,
                  })}
