...
export default function StudentEditProgramaPage() {
  ...
  async function fetchPrograma() {
    ...
    if (data) {
      ...
      setDeadline(data.deadline ?? ''); setVenue(data.venue ?? ''); setNotes(data.notes ?? '')
    }
    setLoading(false)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const { error: err } = await supabase.from('programas')
        .update({ title, type, deadline: deadline || null, venue: venue || null, notes: notes || null })
        .eq('id', programId!)
      if (err) throw err
      toast.success('Programa atualizado!')
      navigate(`/aluno/repertorio/programas/${programId}`)
    } catch (err: unknown) {
